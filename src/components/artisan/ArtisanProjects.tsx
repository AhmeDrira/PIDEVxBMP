...
export default function ArtisanProjects() {
  ...
  const handleAddProjectToPortfolio = async (projectId: string) => {
    try {
      ...
      alert('Project added to portfolio. You can now enrich it with images and videos in Portfolio.');
    } catch (error: any) {
      alert(error?.response?.data?.message || 'Unable to add this project to portfolio.');
    } finally {
