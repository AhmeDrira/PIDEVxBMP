...
export default function CheckoutWizard({ cart, onBack, onCheckout, isLoading, getManufacturerName, updateCartQuantity }: CheckoutWizardProps) {
  ...
  // Pre-fill from user profile in localStorage
  useEffect(() => {
    try {
  ...
  const validateStep = (s: number): boolean => {
    ...
    if (s === 1) {
      if (!data.personalInfo.fullName.trim()) newErrors.fullName = 'Full name is required';
      if (!data.personalInfo.email.trim()) newErrors.email = 'Email is required';
      else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.personalInfo.email)) newErrors.email = 'Invalid email format';
      if (!data.personalInfo.phone.trim()) newErrors.phone = 'Phone number is required';
      else if (!/^[\d\s+()-]{8,}$/.test(data.personalInfo.phone)) newErrors.phone = 'Invalid phone number';
    }

    if (s === 2) {
      if (!data.shippingAddress.address.trim()) newErrors.address = 'Address is required';
      if (!data.shippingAddress.city.trim()) newErrors.city = 'City is required';
      if (!data.shippingAddress.state) newErrors.state = 'State/Governorate is required';
      if (!data.shippingAddress.postalCode.trim()) newErrors.postalCode = 'Postal code is required';
    }
        ...
        return (
          <div key={s.id} className="flex items-center">
            <div className="flex flex-col items-center">
              ...
              <span className={`text-xs mt-1.5 font-medium ${isActive ? 'text-primary' : isCompleted ? 'text-green-600' : 'text-gray-400'}`}>
                {s.label}
              </span>
            </div>
            {i < STEPS.length - 1 && (
              <div className={`w-12 h-0.5 mx-1 mt-[-12px] transition-all duration-300 ${step > i ? 'bg-green-500' : 'bg-gray-200'}`} />
            )}
          </div>
        );
      })}
  ...
  const OrderSummary = ({ compact = false }: { compact?: boolean }) => (
    <Card className="p-5 bg-white rounded-2xl border-0 shadow-lg">
      ...
      <div className="space-y-2 pt-3 border-t border-gray-100">
        ...
        <div className="flex justify-between text-sm text-muted-foreground">
          <span>Shipping</span>
          <span className="font-medium text-foreground">{shippingCost.toFixed(2)} DT</span>
        </div>
          ...
          {cart.map(item => (
            <div key={item._id} className="flex items-center gap-4 p-4 rounded-xl border border-gray-100 hover:border-primary/20 transition-all">
              ...
              <div className="flex items-center gap-2 bg-gray-50 rounded-lg px-2 py-1 border border-gray-200">
                <button
                  type="button"
                  className="w-7 h-7 rounded-md border border-gray-300 bg-white text-sm font-bold hover:bg-gray-100"
      ...
      <div className="space-y-5">
        <div>
          ...
          <Input
            ...
            className={`h-11 rounded-xl $`} style={errors.fullName ? { borderColor: '#ef4444' } : {}}
          />
        ...
        <div>
          ...
          <Input
            ...
            className={`h-11 rounded-xl $`} style={errors.email ? { borderColor: '#ef4444' } : {}}
          />
        ...
        <div>
          ...
          <Input
            ...
            className={`h-11 rounded-xl $`} style={errors.phone ? { borderColor: '#ef4444' } : {}}
          />
      ...
      <div className="space-y-5">
        <div>
          ...
          <Input
            ...
            className={`h-11 rounded-xl $`} style={errors.address ? { borderColor: '#ef4444' } : {}}
          />
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <Input
              ...
              placeholder="Tunis"
              className={`h-11 rounded-xl $`} style={errors.city ? { borderColor: '#ef4444' } : {}}
            />
          ...
          <div>
            ...
            <select
              ...
              className="w-full h-11 rounded-xl border px-3 text-sm bg-white border-input"
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <Input
              ...
              placeholder="1000"
              className={`h-11 rounded-xl $`} style={errors.postalCode ? { borderColor: '#ef4444' } : {}}
            />
    ...
    <Card className="p-6 bg-white rounded-2xl border-0 shadow-lg">
      ...
      <div className="space-y-5">
        <div className="p-4 rounded-xl bg-gray-50 border border-gray-100">
          ...
          <p className="text-sm text-muted-foreground">{data.personalInfo.email} &middot; {data.personalInfo.phone}</p>
        </div>
        ...
        <div className="p-4 rounded-xl bg-gray-50 border border-gray-100">
          ...
          <p className="text-sm text-muted-foreground">{shippingCost.toFixed(2)} DT</p>
        </div>
  ...
  // --- MAIN RENDER ---
  // Steps 1-3 (forms) are centered; steps 0,4 use the sidebar layout
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={isFormStep ? 'max-w-2xl mx-auto' : 'grid lg:grid-cols-3 gap-6'}>
        <div className={isFormStep ? '' : 'lg:col-span-2'}>
          {step === 0 && <CartStep />}
        ...
        {!isFormStep && <div className="hidden lg:block">
          <div className="sticky top-8 space-y-4">
            <OrderSummary compact={step > 0} />
