...
export default function NotificationBell() {
  ...
  const getNotificationIcon = (type: string, isRead: boolean) => {
    const iconClass = isRead ? 'text-gray-400' : 'text-white animate-pulse';
    switch (type) {
      case 'new_order':
  ...
  return (
    <div className="relative z-50 flex items-center justify-center p-2" ref={dropdownRef}>
      ...
      >
        <Bell 
          size={24} 
          className={`transition-transform duration-300 stroke-[2.5px] ${open ? 'rotate-12 text-primary' : 'text-gray-700'}`} 
