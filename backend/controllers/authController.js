...
// Crée le mini site de l'artisan (slug + ArtisanDomain).
// Volontairement non bloquant : un souci sur le mini site ne doit jamais faire
// échouer une inscription. Le slug pourra être rattrapé plus tard (idempotent).
  ...
  try {
    ...
    switch (role) {
      case 'artisan':
        ...
        // Mini site public disponible immédiatement, sans action de l'artisan
      ...
      case 'manufacturer': {
        const mfrData = { ...userData, companyName: '' };
        if (req.file) {
          mfrData.certificationFile = req.file.filename;
        }
        user = await Manufacturer.create(mfrData);
        // Notify admin of new manufacturer registration request
        await Notification.create({
          type: 'manufacturer_registration',
          title: 'New Manufacturer Registration',
          message: `${firstName} ${lastName} (${email}) has submitted a manufacturer account request and is awaiting approval.`,
          relatedId: user._id,
        });
        break;
      }
...
async function createOAuthUser(baseData, role) {
  ...
  switch (newRole) {
    ...
    case 'artisan':
    default: {
      // Même règle que l'inscription classique : le mini site existe dès la création.
