...
describe('artisanRoutes', () => {
  test('configures portfolio multer storage and upload middleware', () => {
    jest.isolateModules(() => {
      ...
      expect(multerLocal.diskStorage).toHaveBeenCalledTimes(2);
      expect(multerLocal).toHaveBeenCalledTimes(2);
      ...
      expect(mockArray).toHaveBeenCalledTimes(4);

      const storage = multerLocal.diskStorage.mock.calls[1][0];
